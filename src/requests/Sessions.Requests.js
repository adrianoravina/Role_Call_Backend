...
const createSession = async (data) => {
    ...
    return await mysqlConnect.promise().query(sql, data)
    .then(([ rows,fields ]) => {
...
const checkIn = async (attendanceCode) => {
    ...
    const sql = "SELECT s_date FROM blocks as b LEFT JOIN sessions as s ON b.b_sessionId = s.session_Id "
    +"where block_id ="+attendanceCode;
...
const createStudentBlock = async (attendanceCode, studentId) => {
    const sql = "INSERT INTO blocks_students (b_s_blockId, b_s_studentId) VALUES(?,?)";
